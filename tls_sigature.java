...
public class tls_sigature {
    final private int mBase64Flag = Base64.NO_WRAP;
     ...
     * @param sdkappid
     */
    public tls_sigature setSdkAppid(int sdkappid) {
     ...
     */
    public tls_sigature setPrivateKey(String privateKey) {
        String privateKeyPEM = privateKey.replace("-----BEGIN PRIVATE KEY-----", "")
                .replace("-----END PRIVATE KEY-----", "")
                .replace("\n", "");
        byte[] encodedKey = Base64.decode(privateKeyPEM.getBytes(), mBase64Flag);
     ...
     */
    public tls_sigature setPublicKey(String publicKey) {
        String publicKeyPEM = publicKey.replace("-----BEGIN PUBLIC KEY-----", "")
                .replace("-----END PUBLIC KEY-----", "")
                .replace("\n", "");
        byte[] encodedKey = Base64.decode(publicKeyPEM.getBytes(), mBase64Flag);
    ...
    private byte[] base64UrlEncode(byte[] data) {
        byte[] encode = Base64.encode(data, mBase64Flag);
    ...
    public String genUserSig(String userid, int expire) {
        ...
        String sig = Base64.encodeToString(signBytes, mBase64Flag);
    ...
    public String genPrivMapEncrypt(String userid, int roomid, int expire) {
        ...
        String userbuf = Base64.encodeToString(bytes, mBase64Flag);
